...
- Normalisasi Ukuran Halaman Lampiran (21 Aug 2026): _compress_pdf_bytes kini menyamakan SEMUA halaman lampiran ke A4 (fit proporsional + center; landscape → A4 landscape) sebelum kompresi. Testing agent iteration_9: 10/10 pass + 28/28 regression invoice. CATATAN: 7 kegagalan pytest pre-existing di suite perangkat_bank (tes usang vs aturan prefix 13 karakter) — bukan bug user-facing, perlu dirapikan nanti.
- Rule Invoice Gabungan (21 Aug 2026): picker kategori invoice kini 2 pilihan — NON_MAINTENANCE (GABUNGAN: WO Survey/Instalasi/Aktivasi/Dismantle boleh satu invoice) & MAINTENANCE (hanya WO maintenance). Fallback kartu "Kategori lama" untuk edit invoice existing berkategori lama. Fix HIGH dari testing agent: create/update invoice kini memvalidasi _wo_matches_activity per WO (400 jika tidak sesuai kategori) + guard double-billing (400 jika WO sudah dipakai invoice lain, exclude invoice sendiri saat edit) via helper _validate_wos_for_invoice. Testing agent iteration_8: frontend 100%, backend pytest tests/test_invoice_non_maintenance.py 10/10 pass setelah fix.
- Fix Build VPS + requirements (21 Aug 2026): requirements.txt tercemar pip-freeze (emergentintegrations/litellm wheel internal → ResolutionImpossible di VPS). Dikembalikan ke daftar terkurasi + pymupdf>=1.26 + cap pandas<3 & reportlab<5. PELAJARAN: JANGAN pip freeze ke requirements.txt di project ini — tambah paket dengan append baris manual. Bonus fix HIGH dari testing agent: duplicate invoice_no kini 409 (bukan 500 DuplicateKeyError) di POST & PUT /invoices. Testing agent iteration_7: bug fix terverifikasi (pip dry-run PyPI murni sukses), regression suite baru di /app/backend/tests/ (18/18 pass).
- Scan Invoice + Kompresi PDF (21 Aug 2026): (1) upload/download/delete /api/invoices/{id}/scan-invoice (field scan_invoice_attachment, PDF maks 20MB) → SELALU jadi halaman PERTAMA PDF Lampiran (urutan: scan → faktur pajak → bukti potong → lampiran WO); UI blok upload di modal invoice (data-testid invoice-form-scan-invoice-file). (2) Kompresi lampiran dengan pymupdf rewrite_images(dpi_target=120, quality=70) + tobytes(garbage/deflate) — teruji reduksi 95% (10.9MB → 505KB). pymupdf==1.28.2 ditambah ke requirements.txt (prod perlu rebuild backend). Teruji e2e via API + screenshot UI.
- FE PUT access (20 Aug 2026): PUT /api/workorders/{id} kini mengizinkan role field_engineer — dibatasi: hanya WO miliknya (403), hanya FE_EDITABLE_FIELDS, exclude_unset (subset yang dikirim saja, tidak menimpa field lain), 400 jika tak ada field valid. Teruji curl 4 skenario (subset OK, 403 WO lain, 400 tanpa field valid, regresi admin PUT full).
- UI tweak (20 Aug 2026): field "CP Pelaksana" (cp_mitra) dihapus dari form WO (schema media section). Field backend & mapping export/import tetap ada untuk data lama. Diverifikasi screenshot.
